  ...
  updateClusters(camera, viewMatrix, scene) {
    ...
    for (let i = 0; i < NUM_LIGHTS; i++){
      ...
      for (xMin = 0; xMin < this._xSlices; xMin++) {
          let xSeg = (-this._xSlices * 0.5 + xMin) * x_step;
          let dist = getDistanceX(lightPos, xSeg);
          if (Math.abs(dist) < lightRadius) break;
      }

      for (xMax = this._xSlices - 1; xMax >= xMin; xMax--){
        let xSeg = (-this._xSlices * 0.5 + xMax) * x_step;
        let dist = getDistanceX(lightPos, xSeg);
        if (Math.abs(dist) < lightRadius) break;
      }

      for (yMin = 0; yMin < this._ySlices; yMin++){
          let ySeg = (-this._ySlices * 0.5 + yMin) * y_step;
          let dist = getDistanceY(lightPos, ySeg);
          if (Math.abs(dist) < lightRadius) break;
      }

      for (yMax = this._ySlices - 1; yMax >= yMin; yMax--){
          let ySeg = (-this._ySlices * 0.5 + yMax) * y_step;
          let dist = getDistanceY(lightPos, ySeg);
          if (Math.abs(dist) < lightRadius) break;
      }
      ...
      for (let x = xMin; x <= xMax; x++){
        for(let y = yMin; y <= yMax; y++){
          for(let z = zMin; z <= zMax; z++){
            ...
            let offset = num_light - Math.floor(num_light / 4.0);
