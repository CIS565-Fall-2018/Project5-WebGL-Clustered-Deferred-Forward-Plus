export default function(params) {
  ...
  float UnpackClusterEX(int x, int y, int z, int index) {
    int indexOver4 = index / 4;
    float u = (float(x + y * CLUSTER_X + z * CLUSTER_X * CLUSTER_Y) + 0.3) / float(CLUSTER_X*CLUSTER_Y*CLUSTER_Z);
    float v = (float(indexOver4) + 0.3) / ceil(float(${params.numLights}) / 4.0);
    vec4 texel = texture2D(u_gbuffers[0], vec2(u, v));
  ...
  void main() {
    ...
    for (int i = 0; i < ${params.numLights}; ++i) {
      float inside = UnpackClusterEX(x, y, z, i);
