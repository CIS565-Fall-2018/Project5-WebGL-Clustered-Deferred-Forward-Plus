export default function(params) {
  ...
  uniform mat4 u_viewMatrixInv;//me
  ...
  void main() {
    ...
    vec4 gb0 = texture2D(u_gbuffers[0], v_uv);//color, normal.x
    vec4 gb1 = texture2D(u_gbuffers[1], v_uv);//position, normal.y

    vec3 fragColor = vec3(0.0);

    vec3 albedo = gb0.rgb;
    vec3 v_position = gb1.rgb;
    vec3 normal = vec3(gb0.w, gb1.w, 0);
    normal.xy = (normal.xy - 0.5) * 2.0;
    normal.z = sqrt(1.0 - dot(normal.xy, normal.xy));
    normal = normalize((u_viewMatrixInv * vec4(normal, 0)).xyz);//camera to world
    ...
    vec4 lightCountCol = vec4(float(lightCount) / float(${params.numLights}), 0, 0, 1);
